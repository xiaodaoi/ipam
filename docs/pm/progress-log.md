...
<!-- 新条目插入到本行下方 -->
## 2026-09-16 · M3-013 补遗——双栈模板表达式改为下拉选项
- **改动**：双栈模板表单「表达式」由自由输入 `Input` 改为 `Select`，只列后端 `ApplyTemplate` 已实现的两种：`{v4.hextet4}`（B 型·十进制镜像，地址可读）与 `{v4.hex32}`（A 型·v4 压入末 32 位，紧凑）。此前手填其它写法会触发 `unsupported expr`。
- **联动**：选择表达式时自动同步「编码」为 B/A（两字段语义重复，避免出现「编码 A + 表达式 hextet4」这类矛盾配置）；新建默认 `{v4.hextet4}`（与默认编码 B 一致），提交前校验表达式必选并给出提示。
- **验证**：下拉选项仅两项且描述清晰；选 hex32 → 编码自动变 A 型，选 hextet4 → 变 B 型；**实测提交 payload 为原始表达式 `"expr":"{v4.hex32}"` 而非选项标签**（关键正确性点）；列表行显示「A 型 / {v4.hex32}」；控制台无错误；vue-tsc 0 error。验证用模板已删除、环境还原。
...
- **修复**：v6.vue 重写选中交互——加载时一次拉全部 v6 在线行（family=6+state=online，不带 subnetId），按 subnetId 计数；自动选中"有在线地址的网段"（无则第一个）；主表新增在线地址数列（>0 绿标）与选中行高亮（bg-accent）；在线卡标题恒显选中 CIDR，列改 在线地址/客户端标识（DUID）/主机名/租期到期；点击网段行切换（在线行由 computed 过滤，零额外请求）。
- **验证**：pnpm build:ipam 全过（11 任务）；容器内 bundle 含改动字符串（防静默缓存）；数据链路上游（绑定/台账 API/日志）已在补遗⑨验证。
- **教训**：build:ipam 必须在 web/ 目录跑（pnpm 工作区根在 web/，仓库根跑 pnpm 会报 NO_IMPORTER_MANIFEST 且 sync-webui 会同步旧产物）。
...
- **需求**：解析记录表增加完整域名记录列表。
- **顺带修复**：fqdn 拼接缺陷——记录名已含区域后缀时（用户填完整域名 work2046.com / www.work2046.com）再追加区域名 → work2046.com.work2046.com. → 查询 NXDOMAIN（现场问题）。fqdn() 与 ExportZonefile 同步修复：名称已含区域后缀只补尾点。
- **前端**：解析记录表加「完整域名」列（fqdnOf：记录名+区域名拼接，含尾点/后缀判断）。
- **验证**：渲染文件 local-data 名字正确（work2046.com. / www.work2046.com.）；dig www.work2046.com → 118.194.233.63 ✓；表格列头含完整域名、行显示 www → www.crphbz.com. ✓。

## 2026-09-07 · M3-012 补遗④——组织管理迁入系统设置页 + 在线表分页20 + v6台账间距
- **组织管理迁入系统设置**：抽取 org-manage-card.vue 共享组件（含拖拽排序/增删改全功能）；系统设置页顶部嵌入该模块；系统管理二级菜单移除组织管理项；删除旧 orgs 页面。
- **在线表分页**：v4/v6 台账在线地址表 antd Table 分页 pageSize 20（showSizeChanger）。
- **v6 台账间距**：补 p-4 外层（对齐 v4/子网管理），在线模块 mt-4。
- **验证**：系统设置页组织卡片+拖拽树渲染 ✓；侧边栏系统管理子项无组织管理 ✓；v4/v6 在线表 pageSize 20 ✓；typecheck/build 绿。

## 2026-09-07 · M3-012 补遗③——台账菜单导航不渲染修复 + 在线模块嵌入台账页 + 间距对齐

- **菜单点击 IPv4/IPv6 台账不渲染（刷新才出来）**：根因——DhcpLedger 父路由只有 redirect 无 component，vben 页签缓存下 SPA 导航时子组件不渲染。修复：parent.vue（仅 RouterView 透传）挂到父路由。实测菜单点击/SPA 导航 v4/v6 页均正常渲染。
- **在线地址改模块**：去掉「在线地址」菜单项；在线模块嵌入 IPv4 台账页（地址规划下方，mt-4 间距对齐子网管理；标题带选中 CIDR 与计数，数据取 loadMap 已拉的台账 online 行）；IPv6 台账页同样嵌入（当前 v6 租约级联动未接入，诚实空态+说明）。
- **顺带修复**：v4.vue 误删 Card 导入导致在线卡片不渲染（unknown element）。
- **验证**：菜单点击/SPA 导航 v4（map+org+在线模块）与 v6（表+在线模块）均渲染；三张卡片（组织/地址规划/在线地址）DOM 确认；在线地址菜单项已移除。

## 2026-09-07 · M3-012 补遗②——台账三级菜单拆分 + 在线地址页 + 子网在线数/CIDR跳转 + 台账统计

- **台账拆三级菜单**：地址台账 → IPv4台账（地图+统计）/ IPv6台账（子网级汇总表）/ 在线地址（全网在线列表+模糊搜索+族筛选）；共享组织筛选组件 org-filter-card 抽取复用；路由 redirect 默认进 v4。
- **在线地址列表**：/ledger?state=online（v4+v6 合并），列：在线地址/族/所属子网(CIDR 关联)/MAC/主机名/租用/到期；支持 IP/MAC/主机名模糊搜索。
- **子网管理**：新增「在线地址数」列（/ledger?state=online 按 subnetId 聚合）；CIDR 列改可点击链接跳转 /dhcp/ledger?cidr=xxx（v4 页读 query.cidr 定位网段）。
- **台账统计栏**：在线/静态/保留/已规划/已选中/总数（替代原 4 项）。
- **顺带修复**：UpdateSubnet 改 PATCH 合并语义（原全量覆盖：空 body 清空 name 撞唯一键 500，且会清空未传字段）；mapSubnetErr 加 23505→409 友好报错；kea-dhcp4/6 compose 加租约持久卷 /var/lib/kea（租约与 dhcp6 server-id 不再随容器重建丢失）；kea 启动前清理陈旧 socket 文件（死 socket 导致 bind 失败——dhcp6 socket 二次丢失的根因）。
- **验证**：三级菜单 DOM 结构确认；v4 台账统计正确（10.99.0.0/24：3静态/4保留/7已规划）；子网列表新列+CIDR 链接点击跳转定位 ✓；在线地址页渲染（无活跃租约时暂无数据为正确语义）；typecheck/build 绿。
...
- **根因**：M3-011 表单重构（删"族"选择、改分族新建）时误删了 CIDR 字段——警告文案还在、输入框没了。
- **修复**：CIDR 输入框恢复（占位符按 family 自适应 v4/v6 示例；网关占位符同步改为 10.61.40.1）。
- **验证**：API 按用户场景创建 10.61.40.0/24（池 10.61.40.100-200、租期 1 天、网关/DNS）→ Kea 运行态确认 subnet+pools+vl+options 全部就位 ✓。池范围提示用户按实际规划调整。
...
- **部署坑再现**：本轮 docker build 后忘执行 up 重建容器，导致"构建了但没生效"假象；且 build 静默缓存问题需盯 web 阶段重跑。
- **验证**：Playwright 量测 w1600：地址规划 1184 = IPv6 网段 1184；w1280：两者 864——与视口同步自适应。
